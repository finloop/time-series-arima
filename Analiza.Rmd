...
which(abs(t.bc.52.1.acf$acf)>1.96/sqrt(t.bc.52.1.acf$n.used)) # Wszystkie lag spoza przedziałem
```

Obliczam współczynniki MA(52) i MA(1):
...
which(abs(t.bc.52.1.pacf$acf)>1.96/sqrt(t.bc.52.1.pacf$n.used)) # Wszystkie lag spoza przedziałem
```

### AR(52) i AR(56)

Obliczam współczynniki AR(52), AR(56), AR(1):
...
Wszystkie modele korzystały z transformacji Boxa-Coxa więc mogę je porównywać między sobą.
    ...
    # ARIMA(1,1,0)(1,1,0)[52] AIC=-343.57   AICc=-343.54   BIC=-329.7  +

Ze wszystkich modeli, najlepszym wydaje się ARIMA(56,0,0). Pomimo dużej ilości, parametrów jako jedyny y przechodzi test Ljung-Boxa. Analiza reszt znajduje się na wykresach poniżej.
